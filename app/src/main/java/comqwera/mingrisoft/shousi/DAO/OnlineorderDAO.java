...
public class OnlineorderDAO {
    ...
    public void add(Onlineorder onlineorder){
        ...
        //执行添加用户信息操作
        db.execSQL ("insert into myuser(or_id,u_id,d_id,or_sum,or_memo) values(?,?,?,?,?)",new Object[]
                {
                        onlineorder.getOr_id (),onlineorder.getU_id (),onlineorder.getD_id (),onlineorder.getOr_sum (),onlineorder.getOr_memo ()//在线预订
                });
    }
    /**
     * 更新用户信息
    ...
    public void update(Onlineorder onlineorder){
        db=helper.getWritableDatabase ();//初始化SQLiteDatabase对象
        //执行修改收入信息操作
        db.execSQL ("update myuser set or_id=?,u_id=?,d_id=?,or_sum=?,or_memo=? where or_id=?",
                new Object[]
                        {
                                onlineorder.getOr_id (),onlineorder.getU_id (),onlineorder.getD_id (),onlineorder.getOr_sum (),onlineorder.getOr_memo ()
                        });
    }
    /**
     * 查找用户信息
    ...
    {
        db=helper.getWritableDatabase ();//初始化SQLiteDatabase对象
        Cursor cursor=db.rawQuery ("select or_id,u_id,d_id,or_sum,or_memo where re_id=? ",
                new String[]
                        {String.valueOf (id)});
